...
- 將文件片段、實體與關係建立 Embedding，寫入 Neo4j 並建立向量索引。
- 支援保留既有資料、取代目前圖譜，以及清空本工具建立的圖譜。
- 使用向量搜尋、圖譜擴展與原文片段組成混合式 GraphRAG 問答內容。
...
2. 從命中的節點向外擴展相關圖譜內容。
3. 回查對應的 PDF 原文片段作為證據。
4. 將問題、圖譜內容及原文證據交給回答模型生成答案。

匯入階段預先建立 Embedding 與向量索引，是後續問答能快速檢索的關鍵。

## 基本使用流程

1. 在「連線設定」填入 Neo4j 與 OpenAI 相容模型服務，分別執行連線測試。
2. 在建圖頁上傳 PDF 並確認解析結果。
3. 選擇全文或隨機 N 頁，執行「分析文件並規劃 Schema」。
4. 視需要修改 Schema，設定 LLM 與最大並行數，再執行「確認 Schema 並抽取知識圖譜」。
5. 選擇資料處理方式，按下「Embedding 並匯入 Neo4j」。
6. 前往問答頁，使用連線設定中的 Neo4j 與模型服務進行提問。
