...
自動問答測試會顯示 Recall@5 與 MRR：Recall@5 代表前 5 筆證據是否命中題目標記的來源 chunk（舊題目沒有 chunk 時改用來源頁碼），MRR 則依第一筆命中證據的排名計算倒數排名平均值。
...
1. 在「0. 專案設定」建立新專案，或載入既有專案；完成前仍可使用「1. 連線設定」，第 2–7 頁會保持鎖定。刪除專案時瀏覽器會再次要求確認，確認後才永久刪除該專案的設定、文件與紀錄。
...
8. 前往「5. 自動問答測試」，指定題數後從 PDF 建立測試集。題目與答案可直接修改，也可匯入 JSON／CSV 或匯出 JSON；修改或匯入後會標示「尚未儲存」，須先按「儲存題目」才能一鍵測試。完成後會以大型摘要顯示答對題數、總題數、答錯題數與正確率。
