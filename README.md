...
上傳 PDF 後可設定「解析起始頁」與「解析結束頁」；結束頁留空時會解析到文件最後一頁。系統會在 PyMuPDF4LLM 階段只處理指定範圍，並驗證起訖順序及頁碼是否超出文件範圍。

解析完成後可使用頁碼滑桿或「上一頁／下一頁」逐頁檢視所選範圍內的所有相關 chunk。預覽區採滿版寬度配置，表格最大高度為 750px，並提供內容搜尋。
