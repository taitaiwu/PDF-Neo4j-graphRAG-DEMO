...
- 依「每份 PDF 生 N 題」自動建立問題、標準答案與來源頁碼／chunk；可設定跨 PDF 的生題並行數與測試並行數（同一份 PDF 內逐題序列生成、並會排除已生成題目以避免重複）。題目支援手動編輯、保存及 JSON／CSV 匯入與 JSON 匯出，再一鍵執行 RAG 回答及模型判分，結果會顯示每份 PDF 與整體的答對題數。
...
2. 在「連線設定」填入 Neo4j 與 OpenAI 相容模型服務，分別執行連線測試；Neo4j 必須連線成功，模型服務與 Embedding 服務任一個測試成功即可解鎖第 2–6 頁（實際建圖與問答仍需要對應功能的服務可用）。
...
7. 前往「4. 自動問答測試」，設定「每份 PDF 題目數 N」與生題／測試最大並行請求數後，從已選取的 PDF 建立測試集（總題數＝文件數 × N；跨 PDF 並行生成，同一份 PDF 內序列生成並排除已產生的題目，生成完成後仍會再檢查一次是否有跨 PDF 重複題目）。題目與答案可直接修改，也可匯入 JSON／CSV 或匯出 JSON；修改或匯入後會標示「尚未儲存」，須先按「儲存題目」才能一鍵測試。完成後會以大型摘要顯示每份 PDF 與整體的答對題數、總題數、答錯題數與正確率。
