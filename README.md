...
7. 前往「4. 自動問答測試」，指定題數後從 PDF 建立測試集。題目與答案可直接修改，也可匯入 JSON／CSV 或匯出 JSON；修改或匯入後會標示「尚未儲存」，須先按「儲存題目」才能一鍵測試。
8. 「5. 問答測試」可進行單題提問；成功結果會自動加入「6. 歷史紀錄」。
9. 所有設定、參數及處理結果會在目前專案中自動保存；每次進入「0. 專案設定」也會自動更新專案清單。

專案資料位於 `data/projects/<project-id>/`。`project.json` 保存設定、建圖狀態、自動測試集、測試結果與問答紀錄，`documents/` 保存 PDF 副本。Password 與 API Key 會以明文保存在本機專案檔，請勿提交或分享 `data/`。
