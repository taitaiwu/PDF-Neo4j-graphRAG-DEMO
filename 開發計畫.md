...
4. 在獨立規劃區設定抽取 LLM、temperature、最大輸出 tokens、schema 粒度及實體／關係類型上限，將所有 chunks 分批產生可泛化且不含具體實例類型的候選 schema，再以較小整合批次及精簡中間欄位進行階層式多輪合併，形成具放大字體與捲動預覽的實體與關係 schema，讓使用者編輯確認；在另一區塊分批處理全部 chunks、顯示帶來源的抽取結果，待使用者確認後再按按鈕交易式匯入 Neo4j。
