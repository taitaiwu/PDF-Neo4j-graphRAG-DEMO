...
4. 在獨立規劃區設定抽取 LLM、temperature 與最大輸出 tokens，將所有 chunks 分批產生候選 schema，再以較小整合批次及精簡中間欄位進行階層式多輪合併，形成具放大字體與捲動預覽的實體與關係 schema，讓使用者編輯確認；在另一區塊分批處理全部 chunks、顯示帶來源的抽取結果並自動交易式匯入 Neo4j。
