...
├── ui/                 # 畫面與事件
├── configs/            # 參數模型與預設值
├── pipeline/           # PDF、chunk、抽取與 embedding
├── services/           # 建圖、問答與結果比較
├── adapters/           # 模型與 Neo4j 連線
├── storage/            # JSON 與檔案讀寫
├── tests/              # 單元及整合測試
├── data/               # 執行資料，不納入版本控制
...
2. 逐頁解析文字，標示空白或疑似掃描頁。
3. 依使用者參數切分 chunk，提供建圖前預覽。
4. 使用選定 LLM 抽取實體與關係，保留來源 chunk。
5. 使用選定 embedding 建立向量。
6. 將文件、chunk、實體、關係與來源寫入 Neo4j。
7. 建立向量及全文索引，驗證完成後將 run 標為 `ready`。
...
所有 JSON 包含 `schema_version`，以暫存檔加原子替換方式寫入。密碼與 API key 只從環境變數或遮罩欄位取得，不寫入 JSON。
