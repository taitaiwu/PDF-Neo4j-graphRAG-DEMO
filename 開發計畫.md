...
4. 在獨立規劃區設定抽取 LLM、temperature、最大輸出 tokens、schema 粒度及實體／關係類型上限及最大並行請求數，可選擇全部頁面或隨機抽取 N 頁，將選定範圍的 chunks 受控並行產生可泛化且不含具體實例類型的候選 schema，等待全部規劃批次完成後，再以較小整合批次及精簡中間欄位逐輪並行進行階層式合併，形成具放大字體與捲動預覽的實體與關係 schema，讓使用者編輯確認；在第二區塊另選抽取 LLM 與最大並行請求數，受控並行完成全部 chunks 後統一去重整合，並顯示進度與帶來源的抽取結果，待使用者確認後，在獨立的第三區塊選擇 Embedding 模型，選擇保留、取代最近一次或清空本工具圖譜後，分批建立原文 chunk、實體與關係向量，再按按鈕交易式匯入 Neo4j；破壞性操作須明確確認。
...
1. 直接連線至設定的 Neo4j，載入最近更新的建圖結果。
2. 匯入時預先建立該 run 的 Neo4j Vector Index；問答時只計算問題 embedding 並執行向量或混合檢索。
3. GraphRAG 模式從命中節點擴展相關實體與關係。
4. 將證據交給問答 LLM 生成答案，顯示來源頁碼、chunk、相似度與檢索內容，並保存 JSON 紀錄。
